...
export async function GET(request: Request) {
  ...
  try {
    ...
    const parsed = workbenchListSchema.parse({
      cursor: url.searchParams.get("cursor"),
      limit: parseOptionalInt(url.searchParams.get("limit"))
    });
    const page = await listWorkbenchProjectsForUser(viewer.user.id, parsed);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const parsed = workbenchProjectCreateSchema.parse({
      title: body.title,
      sortOrder: body.sort_order,
      collapsed: body.collapsed,
      activeSessionId: body.active_session_id
    });
    const project = await createWorkbenchProjectForUser(viewer.user.id, parsed);
