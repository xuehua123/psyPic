...
PsyPic 是一个独立的 GPT Image 商业图片工作台，目标是先交付可公开试用的商业创作台，再逐步扩展到项目工作区、版本流、任务 runtime、灵感社区、同款生成和完整素材/历史能力。
...
- 图片工作台化基础：ProjectSidebar、项目卡、版本流、Inspector、素材库、任务队列和分支/派生语义已经形成，后续按 [docs/19-图片工作台改造蓝图.md](docs/19-图片工作台改造蓝图.md) 收敛。
- Board Mode（多素材多图层画布）方案已完成：将多素材编排、手绘标注、mask 草图、reference PNG 导出和 version node snapshot 纳入工作台，详见 [docs/20-多素材多图层画布（BoardMode）改造方案.md](docs/20-多素材多图层画布（BoardMode）改造方案.md)。
...
- Board Mode 候选：Konva + react-konva
...
项目当前处于 V1.0 代码封板期。核心功能、管理端、社区互动、批量工作流、任务队列基础、运行时配置持久化、图片参数边界、Playwright E2E、移动端 smoke 和生产 smoke/load 脚本已经实现；正式发布前仍需在目标 staging/production 环境完成真实 Sub2API launch ticket、对象存储、migration、压测和回滚演练。V1.0 后的主升级方向是图片工作台平台化，其中 Board Mode（多素材多图层画布）是下一条值得优先验证的创作体验。
