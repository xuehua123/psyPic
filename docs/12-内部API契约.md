...
工作台项目接口主要服务 Project Sidebar、版本流和项目切换。MVP 可以先 local-first，但契约应预留服务端化空间。
...
版本图是工作台的核心结构，而不是历史页面的附属信息。

```http
GET /api/workbench/version-graph?project_id=proj_xxx&session_id=session_xxx
POST /api/workbench/version-graph/fork
POST /api/workbench/version-graph/branch
```
...
Board Mode 第一阶段可以 local-first；服务端化后使用 `/api/workbench/boards`。Board API 只处理 PsyPic 自己的 JSON 模型，不暴露 Konva / Fabric / tldraw 内部对象。
...
`POST /api/workbench/boards` 请求建议：
...
`PATCH /api/workbench/boards/{board_id}` 行为：

- 更新标题、尺寸、背景、图层 JSON、active layer、source asset ids。
- 服务端必须校验当前用户对 `project_id`、`session_id`、`source_asset_ids` 的访问权。
- 图层数量、画布最大像素和单层点数要有上限，避免超大 JSON。

`POST /api/workbench/boards/{board_id}/export` 请求建议：
...
- `reference_png`：扁平化画布，作为 edits 的 `image`。
- `mask_png`：只导出 mask layer 的 alpha PNG，作为 edits 的 `mask`。
- `preview_png`：轻量预览缩略图。

## Job Runtime

`/api/tasks` 仍然可以作为当前任务契约，但建议把它理解成统一的 job runtime 入口，而不是只服务一次生成请求。

```http
GET /api/tasks?project_id=proj_xxx&session_id=session_xxx&status=running
...
命令接口可以只做轻量映射，例如新建项目、打开参数面板、新建 Board Mode、从当前版本分叉、导入素材、进入设置页。
...
board_document_id=board_xxx optional
board_export_asset_id=asset_xxx optional
```
...
- Board Mode 导出的 reference PNG 仍按普通 `image` 上传；Board Mode 导出的 alpha mask 仍按普通 `mask` 上传。
- `board_document_id` 和 `board_export_asset_id` 只作为工作台上下文字段，不改变上游 Images API 参数。
...
- 如果任务属于某个 workbench project / version node，也应原样返回上下文字段，便于 Task Dock 和版本流关联。
...
```text
event: task_started
data: {"task_id":"task_xxx","project_id":"proj_xxx","session_id":"session_xxx","version_node_id":"ver_xxx","status":"running","request_id":"psypic_req_xxx","upstream_request_id":"req_xxx"}

event: partial_image
data: {"task_id":"task_xxx","index":0,"asset_id":"asset_xxx","url":"/api/assets/asset_xxx","format":"png"}

event: completed
data: {"task_id":"task_xxx","project_id":"proj_xxx","session_id":"session_xxx","version_node_id":"ver_xxx","images":[...],"usage":{...},"duration_ms":12000}
...
- 如果任务来自某个 workbench project / session / version node，建议原样返回只读上下文字段，便于从历史回到工作台。
...
```http
GET /api/library?cursor=asset_xxx&limit=30&favorite=true&tag=电商主图&role=background&q=香水
```
...
- `tag` 精确匹配规范化后的标签。
- `role` 可选；支持 `background/subject/decoration/text_effect/texture_light/shape/brand/reference/mask`。
- `q` 匹配 `asset_id`、`task_id`、prompt 和 tags。
- `next_cursor` 使用当前页最后一条 `asset_id`。
- 素材项建议额外返回 `project_id`、`session_id` 和 `version_node_id`，方便从素材回到工作台原上下文。
...
- 管理员可关闭公开发布。
- 如果作品来自某个 project / session / version node，响应建议额外保留这些只读字段，便于从社区回流到原工作台上下文，但不得泄露用户未公开的私有内容。
