...
- 本轮已完成：`v0.5` 任务状态查询/取消接口、同步生成任务记录、创作台任务状态展示、取消、重新生成、partial preview、用户并发限制，`v0.6` 遮罩编辑最小闭环，以及 `v0.7` 服务端历史查询、素材库列表、收藏/标签接口和创作台素材库同步入口。
- 已完成 CI：GitHub Actions 执行 lint、typecheck、test、Prisma validate、build。
- 当前主线：`main`。
- 当前焦点：继续推进 `v0.7` 相册、批量下载、图片详情页和移动端素材库体验；`v0.5` 任务队列仍保留为后续异步任务系统收口项。
...
| `v0.7` | 进行中 | 历史、素材库和相册 | 服务端历史、素材库列表、收藏、标签、持久化模型和创作台同步入口已完成；相册/ZIP/详情页待推进 |
...
- [ ] 批量 ZIP 下载只包含当前用户可访问图片。
- [ ] 图片详情页可从历史和素材库进入，并可继续编辑。
