...
- 本轮已完成：`v0.5` 任务状态查询/取消接口、同步生成任务记录、创作台任务状态展示、取消、重新生成、partial preview、用户并发限制，`v0.6` 遮罩编辑最小闭环，以及 `v0.7` 服务端历史、素材库、收藏/标签、创作台素材库同步、相册、ZIP 下载和图片详情页。
- 已完成 CI：GitHub Actions 执行 lint、typecheck、test、Prisma validate、build。
- 当前主线：`main`。
- 当前焦点：推进 `v0.8` 商业模板中心和 Prompt 助手；`v0.5` 任务队列仍保留为后续异步任务系统收口项。
...
| `v0.8` | 进行中 | 商业模板和 Prompt 助手 | 模板列表 API 和结构化 Prompt 渲染 API 已完成；模板字段编辑、Prompt 优化和收藏待推进 |
...
- [ ] 创作台模板中心支持编辑结构化字段后生成 Prompt。
- [ ] 实现中文 Prompt 优化、结构化补全和图像编辑保留项生成。
- [ ] 实现 Prompt 收藏。
...
- [ ] 用户可在创作台编辑模板字段并应用到 Prompt。
- [ ] Prompt 助手输出不绕过模板约束和安全边界。
