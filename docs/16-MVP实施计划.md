...
- 本轮已完成：`v0.5` 任务状态查询/取消接口、同步生成任务记录、创作台任务状态展示、取消、重新生成、partial preview、用户并发限制，`v0.6` 遮罩编辑最小闭环，以及 `v0.7` 服务端历史查询、素材库列表、收藏和标签接口。
- 已完成 CI：GitHub Actions 执行 lint、typecheck、test、Prisma validate、build。
- 当前主线：`main`。
- 当前焦点：继续推进 `v0.7` 相册、批量下载、图片详情页和前端素材库体验；`v0.5` 任务队列仍保留为后续异步任务系统收口项。
...
| `v0.7` | 进行中 | 历史、素材库和相册 | 服务端历史、素材库列表、收藏、标签和持久化模型已完成；相册/ZIP/详情页待推进 |
