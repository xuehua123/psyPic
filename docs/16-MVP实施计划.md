...
- 当前主线：`main`。
- 当前焦点：`v0.9` 管理端和 `v1.0` 社区互动、批量工作流、多参考图、参数边界已完成，进入封板回归和生产演练。
...
| `v0.5` | 已完成 | 流式和任务系统 | 任务状态、取消、重试、SSE partial preview、并发限制和队列 |
...
| `v0.9` | 已完成 | 运营、计费、风控和社区最小发布 | 社区作品长期模型、发布 API、作品详情 API、同款生成隐私开关、用量汇总 API、运行时限制、公开发布开关、举报、下架、恢复、精选、管理端页面和日志审计 |
...
- [x] 商业尺寸预设能正常约束尺寸。
- [x] 图生图错误提示清楚。
- [x] MVP 验收清单的代码内自动化覆盖项全部通过；真实 Sub2API 一键导入生图仍需 staging 验证。
...
- [x] 引入用户并发限制。
- [x] 引入任务队列。
- [x] 补齐流式和并发限制的 E2E 回归；取消和重试继续由 API/组件回归覆盖。
...
- [x] 建立社区举报长期模型 `CommunityReport`。
- [x] 实现 `POST /api/community/reports` 举报入口。
- [x] 实现 `POST /api/admin/community/works/{work_id}/take-down` 管理员下架入口。
- [x] 管理端举报列表、精选和持久化审核开关。
...
21. [x] v0.9 运营、计费、风控和社区最小发布。
    ...
    - [x] 运行时限制和公开发布开关。
    - [x] 举报和下架入口。
    - [x] 管理端限制开关页面、举报列表和持久化。
22. [x] v1.0 灵感社区和高级工作流。
    - [x] 公开社区作品列表 API。
    - [x] `/community` 灵感社区信息流页面。
    - [x] `/community/works/{work_id}` 公开作品详情页。
    - [x] 社区同款生成前端入口。
    - [x] 点赞、收藏、精选和举报列表体验。
23. [x] V1.0 封板回归。
    - [x] Playwright E2E 覆盖桌面主流程和移动端 smoke。
    - [x] CI 执行 lint、typecheck、test、Prisma validate、build 和 E2E。
    - [x] 生产部署、回滚和 smoke/load 脚本文档已补齐。
