...
- 本轮已完成：`v0.5` 任务状态查询/取消接口、同步生成任务记录、创作台任务状态展示、取消、重新生成、partial preview、用户并发限制，`v0.6` 遮罩编辑最小闭环，以及 `v0.7` 服务端历史查询、素材库列表、收藏/标签接口、创作台素材库同步入口、相册最小模型和 ZIP 下载。
- 已完成 CI：GitHub Actions 执行 lint、typecheck、test、Prisma validate、build。
- 当前主线：`main`。
- 当前焦点：继续推进 `v0.7` 图片详情页、移动端素材库体验和阶段回归；`v0.5` 任务队列仍保留为后续异步任务系统收口项。
...
| `v0.7` | 进行中 | 历史、素材库和相册 | 服务端历史、素材库、收藏、标签、持久化模型、创作台同步入口、相册和 ZIP 已完成；详情页待推进 |
